...
class SyncTemplatesFromFilesService
{
    ...
    public function run()
    {
        ...
        foreach ($files as $group => $templates) {
            if (mb_substr($group, -6) !== '.group') {
                continue;
            }

            $group_name = mb_substr($group, 0, -6); // remove .group
            ...
            if (! isset($existing[$group])) {
                ...
                if (in_array($group_name, ['act', 'css'])) {
                    continue;
                }
            ...
            foreach ($templates as $template) {
                ...
                // Skip hidden ._ files
                if (mb_substr($template, 0, 2) === '._') {
                    continue;
                }
                // If the last occurance is the first position?  We skip that too.
                if (mb_strrpos($template, '.') === false) {
                    continue;
                }

                $ext = mb_strtolower(ltrim(mb_strrchr($template, '.'), '.'));
                if (! in_array('.' . $ext, ee()->api_template_structure->file_extensions)) {
                    continue;
                }

                $ext_length    = mb_strlen($ext) + 1;
                $template_name = mb_substr($template, 0, -$ext_length);
     ...
     */
    protected function saveNewTemplateRevision(Template $template) : void
    {
